...
def test_run_logs_defaults_to_simple_live_view_when_follow_is_disabled(
        ...
        "\n".join(
            (
                ...
                _runtime_event_line(
                    ...
                    payload={"output_length": 84, "turn_duration_ms": 912},
                    session_id="sess-1",
                ),
            )
        )
    ...
    assert any(
        "assistant reply saved | session=sess-1 | chars=84 | turn=912 ms" in line
        for line in outputs
    )
...
def test_run_logs_full_shows_raw_lines_when_follow_is_disabled(tmp_path: Path) -> None:
    ...
    log_path.write_text(
        ...
        + _runtime_event_line(
            event_type="assistant.reply.persisted",
            message="Assistant reply persisted.",
            payload={"output_length": 21, "turn_duration_ms": 640},
            session_id="sess-2",
        )
        + "\n",
        encoding="utf-8",
...
def test_render_simple_log_line_shows_tool_durations_and_failures() -> None:
    rendered = render_simple_log_line(
        _runtime_event_line(
    ...
    assert rendered == (
        "[2026-03-13 12:00:00] tool failed | read_text_file | "
        "duration=7 ms | chars=0 | error=File does not exist."
    )
